...
int main() {
  ...
  std::cout << "  name  = " << nameVal->text
            << " (type: " << ir::valueTypeToString(nameVal->tag.type) << ")\n";
  ...
  for (const auto &f : person->fields) {
    auto v = std::static_pointer_cast<ir::NodeScalar>(f.value);
  ...
  auto s1 = ir::makeValue();
  s1->text = "95";
  s1->tag.type = ir::ValueType::I;
  s1->tag.desc = "语文";
  auto s2 = ir::makeValue();
  s2->text = "87";
  s2->tag.type = ir::ValueType::I;
  s2->tag.desc = "数学";
  auto s3 = ir::makeValue();
  s3->text = "92";
  s3->tag.type = ir::ValueType::I;
  s3->tag.desc = "英语";
  ...
  for (size_t i = 0; i < scores->items.size(); i++) {
    auto v = std::static_pointer_cast<ir::NodeScalar>(scores->items[i]);
    std::cout << "    [" << i << "] " << (v ? v->text : "?") << "\n";
  }
  ...
  std::cout << "  解码成功: 对象有 " << decodedObj->fields.size()
            << " 个字段\n";
