...
static void print_tag(const mm_tag_t *tag) {
  ...
  if (tag->desc)
    printf(" desc=\"%s\"", tag->desc);
  if (tag->min)
    printf(" min=%s", tag->min);
  if (tag->max)
    printf(" max=%s", tag->max);
  if (tag->nullable)
    printf(" nullable");
  if (tag->allow_empty)
    printf(" allow_empty");
  ...
  if (tag->raw)
    printf(" raw");
  if (tag->version > 0)
    printf(" version=%d", tag->version);
  if (tag->enums)
    printf(" enum=%s", tag->enums);
  if (tag->pattern)
    printf(" pattern=\"%s\"", tag->pattern);
  if (tag->default_val)
    printf(" default=%s", tag->default_val);
  if (tag->mime)
    printf(" mime=%s", tag->mime);
  if (tag->location_offset != INT_MIN)
    printf(" location=%d", tag->location_offset);
  printf("\n");
}
...
int main(void) {
  ...
  mm_node_t *v5 = mm_str("*raw*", .deprecated = true, .desc = "原始数据");
  printf("  raw str:\n");
  ...
  mm_node_t *v8 =
      mm_str("abc123", .pattern = "^[a-z0-9]+$", .desc = "字母数字");
  printf("  with pattern:\n");
  ...
  mm_node_t *v11 =
      mm_value_create_str("a1b2c3d4-e5f6-7890-abcd-ef1234567890", MM_VALUE_UUID,
                          (mm_field_attr_t){.desc = "UUID"});
  printf("  uuid:\n");
  print_tag(&v11->data.value.tag);
  mm_node_free(v11);

  mm_node_t *v12 = mm_value_create_str("user@example.com", MM_VALUE_EMAIL,
                                       (mm_field_attr_t){.desc = "电子邮箱"});
  printf("  email:\n");
  print_tag(&v12->data.value.tag);
  mm_node_free(v12);

  mm_node_t *v13 = mm_value_create_str("https://example.com", MM_VALUE_URL,
                                       (mm_field_attr_t){.desc = "网址"});
  printf("  url:\n");
  print_tag(&v13->data.value.tag);
  mm_node_free(v13);

  mm_node_t *v14 = mm_value_create_str("192.168.1.1", MM_VALUE_IP,
  ...
  printf("    child_min=%s  child_max=%s\n", arr->data.array.tag.child_min,
         arr->data.array.tag.child_max);
