...
- 현재 인코딩은 최대 65535바이트(64KB)까지 지원합니다. 문서 타입에 대한 완전한 지원이 추가되면 이 제한이 확장될 수 있습니다
- 이 프로젝트는 개발 및 테스트 중이며 아직 프로덕션 환경에서 사용하기에 권장되지 않습니다
- API 및 동작은 변경될 수 있으므로 버전 업데이트를 확인하세요
...
`type=` 태그를 사용하여 데이터 타입을 정의합니다. 형식은 `type=typeIdentifier`입니다. 예: `type=i`는 정수를 의미합니다.

- doc: 인코딩은 최대 65535바이트(64KB)까지 지원합니다. 문서 타입의 완전한 지원 후 이 제한을 초과할 수 있습니다
- vec: 동적 배열/슬라이스, 복합 타입을 허용하지 않습니다
- arr: 배열(고정 길이), 복합 타입을 허용하지 않습니다
- obj: 객체/구조체, 복합 구조이며 다중 언어 struct/object에 대응
- map: 맵 키는 문자열이어야 하며 값은 복합 타입이어서는 안 됩니다
- str: 문자열
- bytes: 바이트 배열
- bool: boolean
- i: int; 정수 리터럴에는 소수점을 포함해서는 안 됩니다
- i8: int8
- i16: int16
- i32: int32
- i64: int64
...
- u32: uint32
- u64: uint64
- f32: float32; 부동 소수점은 NaN/Inf/-0을 지원하지 않습니다. 부동 소수점 리터럴에는 소수점이 있어야 합니다(예: 0.0)
- f64: float64; 부동 소수점은 NaN/Inf/-0을 지원하지 않습니다. 부동 소수점 리터럴에는 소수점이 있어야 합니다(예: 0.0)
- bigint: bigint
- datetime: 기본 UTC 1970-01-01 00:00:00
- date: 1970-01-01
- time: 00:00:00
- uuid: 고유 식별자
- decimal: 10진 소수, 문자열로 전달해야 합니다
- ip: IP, IPv4/IPv6 지원
- url: URL, 표준 URL 형식
- email: 이메일, 표준 이메일 형식
