...
def test_encode_i():
    ...
    b = enc.encode(Val(0, '0', Tag(type=ValueType.I)))
    assert b[0] == 0x20  # PositiveInt | 0
    
    b = enc.encode(Val(5, '5', Tag(type=ValueType.I)))
    assert b[0] == 0x25  # PositiveInt | 5
    
    b = enc.encode(Val(23, '23', Tag(type=ValueType.I)))
    assert b[0] == 0x37  # PositiveInt | 23
    
    # Medium int (1 extra byte)
    b = enc.encode(Val(24, '24', Tag(type=ValueType.I)))
    assert b[0] == 0x38  # PositiveInt | IntLen1Byte (24)
    assert b[1] == 24
    
    b = enc.encode(Val(255, '255', Tag(type=ValueType.I)))
    assert b[0] == 0x38
    assert b[1] == 255
    
    # Large int (2 extra bytes)
    b = enc.encode(Val(256, '256', Tag(type=ValueType.I)))
    assert b[0] == 0x39  # PositiveInt | IntLen2Byte (25)
    assert len(b) == 3
    
    # Negative int
    b = enc.encode(Val(-1, '-1', Tag(type=ValueType.I)))
    ...
    # Edge cases
    b = enc.encode(Val(9223372036854775807, '9223372036854775807', Tag(type=ValueType.I)))
    assert dec(b).decode() == 9223372036854775807
    
    b = enc.encode(Val(-9223372036854775808, '-9223372036854775808', Tag(type=ValueType.I)))
...
def test_encode_float():
    ...
    for i in range(1, 8):
        val = i / 10
        b = enc.encode(Val(val, str(val), Tag(type=ValueType.F64)))
    ...
    # Larger floats
    val = 3.14
    b = enc.encode(Val(val, '3.14', Tag(type=ValueType.F64)))
    result = dec(b).decode()
...
def test_encode_object():
    ...
    obj = Obj(fields=[
        Field(key='meta', value=Obj(fields=[
            Field(key='count', value=Val(5, '5', Tag(type=ValueType.I))),
        ])),
    ])
    b = enc.encode(obj)
    result = dec(b).decode()
...
def test_encode_array():
    ...
    # Nested array
    arr = Arr(items=[
        Arr(items=[Val(1, '1', Tag(type=ValueType.I))]),
        Arr(items=[Val(2, '2', Tag(type=ValueType.I))]),
    ])
    b = enc.encode(arr)
    result = dec(b).decode()
    assert result == [[1], [2]]
...
def test_roundtrip_complex():
    ...
    obj = Obj(fields=[
        ...
        Field(key='meta', value=Obj(fields=[
            ...
            Field(key='count', value=Val(100, '100', Tag(type=ValueType.I))),
        ])),
