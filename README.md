...
- 數組元素必須換行
- null 只允許表示整個文檔是null，不能在字段中使用null
...
- url: 网址，符合标准URL格式
- email: 邮箱，符合标准邮箱格式
- enums: 枚舉，枚舉值是一個字符串，以`|`分割
- media: 媒體，图片、音视频、文件二进制。底層是bytes

### jsonc 示例

mm可以輸出為JSONC等文本格式，也可以從JSONC等文本格式導入，示例如下：
...
| 类型 | 允许出现位置 | 对应类型 | 允许标签 |
| ---- | ---- | ---- | ---- |
| Obj | field | `struct` | `example`</br>`deprecated`</br>`name`</br>`desc`</br>`allow_empty` |
...
- `NewEncoder(w io.Writer) Encoder`: 創建編碼器，一般用不到
- `EncodeFromValue(in any) ([]byte, error)`: 從結構體編碼
- `EncodeFromJsonc(in string) ([]byte, error)`: 從 JSONC 字符串編碼
- `NewDecoder(r io.Reader) Decoder`: 創建解碼器，一般用不到
- `DecodeToValue(in []byte, out any) error`: 解碼到結構體
- `DecodeToJsonc(in []byte) (string, error)`: 解碼到 JSONC 字符串

- `ValueToJsonc(value any, tag string) (jsonc string, err error)`: 從結構體編碼為 JSONC 字符串
- `JsoncToValue(jsonc string, value any) (err error)`: 從 JSONC 字符串解碼到結構體
