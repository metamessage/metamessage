...
天生適合 AI 的理解和交互需求，解決了理解歧義問題。它會完全取代接口文檔、格式約定、版本同步這些傳統協作方式，讓數據本身具備完整的自解釋能力，獨立演化且不影響任何端的解析，從接口交互、配置管理到 AI 數據交互，所有涉及數據流轉的場景。
...
[meta-message](https://github.com/lizongying/meta-message)

注意目前還有很多bug待修復，測試也不完整
...
[releases](https://github.com/lizongying/meta-message/releases/latest)
...
支持 Go、Java、TypeScript。

```bash
./mm -generate -lang go -in input.jsonc -out struct.go
```

```bash
./mm -generate -lang java -in input.jsonc -out Struct.java
```

```bash
./mm -generate -lang ts -in input.jsonc -out struct.ts
```
...
- -lang, -l: 生成目標語言（僅用於 -generate none，支持 go, java, ts）
...
```bash
go get github.com/lizongying/meta-message/pkg
```
...
import (
    "fmt"
    "github.com/lizongying/meta-message/pkg"
)
