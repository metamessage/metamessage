...
// go test -v -run TestValueToMM
//
// go test internal/jsonc/*.go -v -run TestValueToMM/empty_slice_int

// go test internal/jsonc/*.go -v -run TestValueToMMA

func TestValueToMM(t *testing.T) {
	type Address struct {
...
func TestValueToMM_InvalidInput(t *testing.T) {
...
func TestValueToMMA(t *testing.T) {
	type A struct {
		Arr []int `mm:"desc=Arr desc; child_desc=Arr item desc"`
	}

	astNode, err := ValueToMM(A{Arr: []int{1, 2, 3}}, "")
	if err != nil {
		t.Error("err:", err)
	}
...
func TestValueToMMB(t *testing.T) {

	// astNode, err := ValueToMM(A{Arr: B{ArrB: 5}}, "")
	astNode, err := ValueToMM(A{}, "")
	if err != nil {
		t.Error("err:", err)
	}
	fmt.Println("node", Dump(astNode))
