...
inline std::string toString(uint8_t v)                  { return std::to_string(static_cast<unsigned>(v)); }
inline std::string toString(uint16_t v)                 { return std::to_string(v); }
inline std::string toString(int8_t v)                   { return std::to_string(static_cast<int>(v)); }
...
template<> inline int32_t fromString<int32_t>(const std::string& s) { return std::stoi(s); }
template<> inline int64_t fromString<int64_t>(const std::string& s) { return std::stoll(s); }
template<> inline uint32_t fromString<uint32_t>(const std::string& s) { return static_cast<uint32_t>(std::stoul(s)); }
template<> inline uint64_t fromString<uint64_t>(const std::string& s) { return std::stoull(s); }
template<> inline uint8_t fromString<uint8_t>(const std::string& s) { return static_cast<uint8_t>(std::stoul(s)); }
template<> inline uint16_t fromString<uint16_t>(const std::string& s) { return static_cast<uint16_t>(std::stoul(s)); }
template<> inline int8_t fromString<int8_t>(const std::string& s) { return static_cast<int8_t>(std::stoi(s)); }
template<> inline int16_t fromString<int16_t>(const std::string& s) { return static_cast<int16_t>(std::stoi(s)); }
...
template<> inline double fromString<double>(const std::string& s) { return std::stod(s); }
...
#define MM_DESC_FIELD(field, type, ...) \
...
#define MM_FROM_FIELD(field, type, ...) \
    if (_f.key == #field) { \
        ...
        if (_v) obj.field = mmc::ir::detail::fromString<MM_CPP_TYPE(type)>(_v->text); \
    } else
    ...
    if (_f.key == #field) { \
        ...
        if (_a) { \
            for (auto& _item : _a->items) { \
                ...
                if (_iv) obj.field.push_back(mmc::ir::detail::fromString<MM_CPP_TYPE(childType)>(_iv->text)); \
            } \
        } \
    } else
...
#define MM_OBJECT(structName, fieldsMacro) \
    ...
    static const MM_CONCAT(FieldDescriptor, structName) MM_CONCAT(_mm_fields_, structName)[] = { \
        fieldsMacro(MM_DESC_FIELD) \
    }; \
    \
    static constexpr size_t MM_CONCAT(_mm_field_count_, structName) = \
        sizeof(MM_CONCAT(_mm_fields_, structName)) / sizeof(MM_CONCAT(FieldDescriptor, structName)); \
    \
    /* ---- _mm_build_field_tag_ helper (no child_*) ---- */ \
    inline ir::Tag MM_CONCAT(_mm_build_field_tag_, structName)(const MM_CONCAT(FieldDescriptor, structName)& fd) { \
    ...
    inline std::shared_ptr<ir::Object> MM_CONCAT(_mm_to_node_, structName)(const structName& obj) { \
        ...
        size_t _idx = 0; \
        auto _mm_build_field_tag = [](const MM_CONCAT(FieldDescriptor, structName)& fd) -> ir::Tag { \
            return MM_CONCAT(_mm_build_field_tag_, structName)(fd); \
        }; \
        fieldsMacro(MM_TO_FIELD) \
        return _node; \
    } \
    \
...
//
//   Pass 1: generate FieldDescriptor array via MM_DESC_FIELD / MM_DESC_ARRAY_FIELD
//   Pass 2: generate _mm_to_node_    via MM_TO_FIELD   / MM_TO_ARRAY_FIELD
//   Pass 3: generate _mm_from_node_  via MM_FROM_FIELD / MM_FROM_ARRAY_FIELD
// ---------------------------------------------------------------------------
#define MM_MAP(structName, fieldsMacro) \
    ...
    inline ir::Tag MM_CONCAT(_mm_build_field_tag_, structName)(const MM_CONCAT(FieldDescriptor, structName)& fd) { \
        ...
        if (fd.tag.version > 0) tag.version = fd.tag.version; \
        if (fd.tag.mime[0]) tag.mime = fd.tag.mime; \
        if (fd.tag.child_type[0]) tag.childType = ir::parseValueType(fd.tag.child_type); \
        if (fd.tag.child_desc[0]) tag.childDesc = fd.tag.child_desc; \
        if (fd.tag.child_default_val[0]) tag.childDefault = fd.tag.child_default_val; \
        if (fd.tag.child_min >= 0) tag.childMin = std::to_string(fd.tag.child_min); \
        if (fd.tag.child_max >= 0) tag.childMax = std::to_string(fd.tag.child_max); \
        if (fd.tag.child_size > 0) tag.childSize = fd.tag.child_size; \
        if (fd.tag.child_enum_val[0]) tag.childEnum = fd.tag.child_enum_val; \
        if (fd.tag.child_pattern[0]) tag.childPattern = fd.tag.child_pattern; \
        if (fd.tag.child_nullable) tag.childNullable = true; \
        if (fd.tag.child_raw) tag.childRaw = true; \
        if (fd.tag.child_allow_empty) tag.childAllowEmpty = true; \
        if (fd.tag.child_unique) tag.childUnique = true; \
        if (fd.tag.child_version > 0) tag.childVersion = fd.tag.child_version; \
        if (fd.tag.child_mime[0]) tag.childMime = fd.tag.child_mime; \
        return tag; \
    } \
    \
    /* ---- Pass 2: _mm_to_node_ ---- */ \
    inline std::shared_ptr<ir::Object> MM_CONCAT(_mm_to_node_, structName)(const structName& obj) { \
        ...
        fieldsMacro(MM_TO_FIELD, MM_TO_ARRAY_FIELD) \
        return _node; \
    } \
    \
    /* ---- Pass 3: _mm_from_node_ ---- */ \
