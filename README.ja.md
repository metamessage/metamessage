...
- type: データタイプ。テキスト形式では、曖昧でない場合は文字列、整数（int）、小数（float64）、オブジェクト（または類似構造）などに明示的な型タグは不要です。プログラミング言語では、オブジェクト（または類似構造）とマップが判別できれば、マップも型タグ不要です
...
- allow_empty: ブール型を除き、他のタイプはデフォルトで空を許可しません。allow_empty が設定されると、いくつかのルールに従ってを許可します
- unique: スライスまたは配列にのみ適用。要素の重複がないことを示します
- default: デフォルト値。まだ有効ではありません
- example: 配列やマップが空の場合に使用されるサンプルデータ
