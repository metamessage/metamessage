...
La misma lógica se aplica a otros lenguajes

```go
// Enfoque incorrecto
// ID ya es de tipo int64, no es necesario especificar type=i64
// En mm, no usar etiquetas json juntas; mm maneja el nombrado automáticamente, usa name= en mm si es necesario
// Age debería usar el tipo nativo uint8, por lo que se puede omitir type=u8
type User struct {
...
// Enfoque correcto
// Email no tiene tipo nativo, por lo que se requiere type=email
type User struct {
...
// La etiqueta de nivel raíz se puede especificar aquí
tag := "desc=User"
