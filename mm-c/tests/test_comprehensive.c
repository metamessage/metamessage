...
static void test_single_value_roundtrip(void) {
  ...
  printf("  int roundtrip: ");
  {
    ...
    TEST("text is 42",
         dec->data.value.text && strcmp(dec->data.value.text, "42") == 0);
  ...
  printf("  str roundtrip: ");
  {
    ...
    TEST("type is value", dec->type == MM_NODE_VALUE);
    TEST("text is hello",
         dec->data.value.text && strcmp(dec->data.value.text, "hello") == 0);
    ...
    if (dec) {
      ...
      if (dec->type == MM_NODE_OBJECT) {
        ...
        if (dec->data.object.field_count == 2) {
          TEST("field 0 key is name",
               strcmp(dec->data.object.fields[0].key, "name") == 0);
          ...
          TEST("name text Alice",
               name_val->data.value.text &&
                   strcmp(name_val->data.value.text, "Alice") == 0);
          TEST("age text 30", age_val->data.value.text &&
                                  strcmp(age_val->data.value.text, "30") == 0);

          TEST("name tag desc 姓名",
               name_val->data.value.tag.desc &&
                   strcmp(name_val->data.value.tag.desc, "姓名") == 0);
          TEST("name tag min 1",
               name_val->data.value.tag.min &&
                   strcmp(name_val->data.value.tag.min, "1") == 0);
          TEST("name tag max 64",
               name_val->data.value.tag.max &&
                   strcmp(name_val->data.value.tag.max, "64") == 0);

          TEST("age tag desc 年龄",
               age_val->data.value.tag.desc &&
                   strcmp(age_val->data.value.tag.desc, "年龄") == 0);
          TEST("age tag min 0",
               age_val->data.value.tag.min &&
                   strcmp(age_val->data.value.tag.min, "0") == 0);
          TEST("age tag max 150",
               age_val->data.value.tag.max &&
                   strcmp(age_val->data.value.tag.max, "150") == 0);
        }
      }
    }
    mm_node_free(person);
  ...
  printf("  nested object: ");
  {
    ...
    if (dec && dec->type == MM_NODE_OBJECT) {
      ...
      if (dec->data.object.field_count == 2) {
        ...
        TEST("address is object",
             addr_val != NULL && addr_val->type == MM_NODE_OBJECT);
        if (addr_val && addr_val->type == MM_NODE_OBJECT) {
          ...
          if (addr_val->data.object.field_count == 1) {
            TEST("street text",
                 strcmp(addr_val->data.object.fields[0].value->data.value.text,
                        "123 Main St") == 0);
          }
        }
      }
...
static void test_array_roundtrip(void) {
  ...
  printf("  simple int array: ");
  {
    ...
    if (dec && dec->type == MM_NODE_ARRAY) {
      ...
      if (dec->data.array.item_count == 3) {
        TEST("item 0 is 1",
             strcmp(dec->data.array.items[0]->data.value.text, "1") == 0);
        TEST("item 1 is 2",
             strcmp(dec->data.array.items[1]->data.value.text, "2") == 0);
        TEST("item 2 is 3",
  ...
  printf("  mixed array: ");
  {
    ...
    if (dec && dec->type == MM_NODE_ARRAY) {
      ...
      if (dec->data.array.item_count == 3) {
        TEST("item 0 str",
             dec->data.array.items[0]->data.value.tag.type == MM_VALUE_STR);
        TEST("item 1 int",
             dec->data.array.items[1]->data.value.tag.type == MM_VALUE_I);
        TEST("item 2 bool",
  ...
  printf("  array in object: ");
  {
    ...
    if (dec && dec->type == MM_NODE_OBJECT) {
      ...
      if (dec->data.object.field_count == 2) {
        ...
        TEST("scores is array",
             scores != NULL && scores->type == MM_NODE_ARRAY);
...
static void test_tag_attributes_roundtrip(void) {
  ...
  printf("  enum: ");
  {
    mm_node_t *v = mm_i(0, .enum_val = "a|b|c");
    mm_buffer_t *enc = mm_encode(v);
    mm_node_t *dec = mm_decode(enc);
    if (dec) {
      TEST("enum preserved",
           dec->data.value.tag.enum_val &&
               strcmp(dec->data.value.tag.enum_val, "a|b|c") == 0);
    }
    mm_node_free(v);
    if (dec)
      mm_node_free(dec);
    ...
    if (dec && dec->type == MM_NODE_ARRAY) {
      TEST("child_desc preserved",
           dec->data.array.tag.child_desc &&
               strcmp(dec->data.array.tag.child_desc, "child items") == 0);
      TEST("child_min preserved",
           dec->data.array.tag.child_min &&
               strcmp(dec->data.array.tag.child_min, "1") == 0);
      TEST("child_max preserved",
           dec->data.array.tag.child_max &&
               strcmp(dec->data.array.tag.child_max, "10") == 0);
      if (dec->data.array.item_count > 0) {
        TEST("child type inherited",
...
static void test_value_type(void) {
  printf("\n=== ValueType Tests ===\n");
  TEST("Str to string",
       strcmp(mm_value_type_to_string(MM_VALUE_STR), "str") == 0);
  ...
  TEST("Unknown to string",
       strcmp(mm_value_type_to_string(MM_VALUE_UNKNOWN), "unknown") == 0);
}
...
static void test_tag(void) {
  ...
  mm_tag_t parsed =
      mm_tag_parse("// mm: desc=\"姓名\"; min=1; max=64; type=str");
  ...
  mm_tag_t parsed_child = mm_tag_parse(
      "// mm: child_desc=\"items\"; child_min=1; child_max=10; child_type=i");
  TEST("Parsed child_desc", parsed_child.child_desc &&
                                strcmp(parsed_child.child_desc, "items") == 0);
  TEST("Parsed child_min",
       parsed_child.child_min && strcmp(parsed_child.child_min, "1") == 0);
  TEST("Parsed child_max",
       parsed_child.child_max && strcmp(parsed_child.child_max, "10") == 0);
