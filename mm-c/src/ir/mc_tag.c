...
static void encode_string(uint8_t **buf, size_t *cap, size_t *len, int key,
                          const char *s) {
  size_t slen = strlen(s);
  size_t needed;
  if (slen <= 5) {
...
static void encode_string_simple(uint8_t **buf, size_t *cap, size_t *len,
...
static void encode_u64(uint8_t **buf, size_t *cap, size_t *len, int key,
                       uint64_t uv) {
  size_t needed;
  if (uv <= 0xFFULL) {
    needed = 2;
...
static uint8_t parse_mime(const char *s) {
  ...
  if (strcmp(lower, "image/jpeg") == 0)
    return 1;
  if (strcmp(lower, "image/jpg") == 0)
    return 1;
  if (strcmp(lower, "image/png") == 0)
    return 2;
  if (strcmp(lower, "image/gif") == 0)
    return 3;
  if (strcmp(lower, "image/webp") == 0)
    return 4;
  if (strcmp(lower, "image/svg+xml") == 0)
    return 5;
  if (strcmp(lower, "image/avif") == 0)
    return 6;
  if (strcmp(lower, "image/bmp") == 0)
    return 7;
  if (strcmp(lower, "image/x-icon") == 0)
    return 8;
  if (strcmp(lower, "image/tiff") == 0)
    return 9;
  if (strcmp(lower, "image/heic") == 0)
    return 10;
  if (strcmp(lower, "image/heif") == 0)
    return 11;
  if (strcmp(lower, "text/plain") == 0)
    return 12;
  if (strcmp(lower, "text/html") == 0)
    return 13;
  if (strcmp(lower, "text/css") == 0)
    return 14;
  if (strcmp(lower, "text/javascript") == 0)
    return 15;
  if (strcmp(lower, "application/javascript") == 0)
    return 15;
  if (strcmp(lower, "application/json") == 0)
    return 16;
  if (strcmp(lower, "text/csv") == 0)
    return 17;
  if (strcmp(lower, "text/markdown") == 0)
    return 18;
  if (strcmp(lower, "application/pdf") == 0)
    return 19;
  if (strcmp(lower, "application/zip") == 0)
    return 20;
  if (strcmp(lower, "application/gzip") == 0)
    return 21;
  if (strcmp(lower, "application/x-tar") == 0)
    return 22;
  if (strcmp(lower,
             "application/"
             "vnd.openxmlformats-officedocument.spreadsheetml.sheet") == 0)
    return 23;
  if (strcmp(lower,
             "application/"
      ...
      0)
    return 24;
  if (strcmp(lower, "application/octet-stream") == 0)
    return 25;
  if (strcmp(lower, "video/mp4") == 0)
    return 26;
  if (strcmp(lower, "video/webm") == 0)
    return 27;
  if (strcmp(lower, "video/mov") == 0)
    return 28;
  if (strcmp(lower, "audio/mpeg") == 0)
    return 29;
  if (strcmp(lower, "audio/wav") == 0)
    return 30;
  if (strcmp(lower, "audio/flac") == 0)
    return 31;
  if (strcmp(lower, "font/woff2") == 0)
    return 32;
  if (strcmp(lower, "font/ttf") == 0)
    return 33;
...
void mm_tag_inherit(mm_tag_t *tag, const mm_tag_t *parent) {
  ...
  if (parent->child_default_val) {
    free(tag->default_val);
    tag->default_val = strdup(parent->child_default_val);
  }
...
char *mm_tag_to_string(const mm_tag_t *tag) {
  ...
  if (tag->child_default_val) {
    ...
    append_str(&buf, &cap, &len, tag->child_default_val);
    first = false;
  }
  ...
  if (tag->child_enums) {
    ...
    append_str(&buf, &cap, &len, tag->child_enums);
    first = false;
  }
  ...
  *count = 0;
  ...
  for (size_t i = 0; i <= len; i++) {
    ...
    if (!in_quote && (i == len || tag_str[i] == ';')) {
      ...
      while (part_len > 0 && tag_str[start] == ' ') {
        start++;
        part_len--;
      }
      while (part_len > 0 && tag_str[start + part_len - 1] == ' ')
        part_len--;
      if (part_len > 0) {
        if (*count >= capacity) {
          capacity *= 2;
...
mm_tag_t mm_tag_parse(const char *tag_str) {
  ...
  for (int i = 0; i < part_count; i++) {
    ...
    if (strcmp(k, "name") == 0) {
      ...
      r.unique = true;
    } else if (strcmp(k, "default_val") == 0) {
      free(r.default_val);
      ...
      r.child_unique = true;
    } else if (strcmp(k, "child_default_val") == 0) {
      free(r.child_default_val);
      r.child_default_val = strdup(val);
...
uint8_t *mm_tag_bytes(const mm_tag_t *tag, size_t *out_len) {
  ...
  if (tag->child_pattern) {
    encode_string_simple(&buf, &cap, &len, MM_TAG_KCHILDPATTERN,
                         tag->child_pattern);
  }
  ...
  if (tag->child_location_offset != 0) {
    char loc_buf[16];
    int loc_len =
        snprintf(loc_buf, sizeof(loc_buf), "%d", tag->child_location_offset);
    size_t l = (size_t)loc_len;
    if (len + 1 + l > cap) {
      cap = cap ? cap * 2 : 128;
      while (len + 1 + l > cap)
  ...
  if (tag->child_version != MM_TAG_DEFAULT_VERSION) {
    encode_u64(&buf, &cap, &len, MM_TAG_KCHILDVERSION,
               (uint64_t)tag->child_version);
  }
...
void mm_tag_merge(mm_tag_t *dst, const mm_tag_t *src) {
  ...
  if (src->child_default_val) {
    free(dst->child_default_val);
    dst->child_default_val = strdup(src->child_default_val);
  }
  ...
  if (src->child_enums) {
    free(dst->child_enums);
    dst->child_enums = strdup(src->child_enums);
  }
