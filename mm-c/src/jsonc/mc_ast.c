...
static void free_tag_enums(MMC_Tag *tag) {
  if (tag != NULL && tag->enums != NULL) {
    free(tag->enums);
    tag->enums = NULL;
  }
}
...
void mmc_node_free(MMC_Node *node) {
  ...
  case MMC_NODE_OBJECT:
    free_tag_enums(&node->data.object.tag);
    ...
    break;
  case MMC_NODE_ARRAY:
    free_tag_enums(&node->data.array.tag);
